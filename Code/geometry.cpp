  ...
  // vert_pos(*vertices), face_indices(*indices)
{
  ...
  //
  // Create Vertex
  vert_value.resize(n_verts, vec3(0.0f));
  ...
  for (int id = 0; id < n_verts ; id++)
  {
    ...
    vert_value[id] = vert_pos;
    std::cout << "  " << &vert_pos << std::endl;
    std::cout << "  " << &vert_value[id] << std::endl;

    Vertex vert(id, &vert_value);
    std::cout << "  " << id << "  " << &vert_value.at(id) << std::endl;
...
void Geometry::set_Buffers()
{
  ...
  glBufferData(GL_ARRAY_BUFFER, vert_value.size() * 3 * sizeof(float), &vert_value[0], GL_STATIC_DRAW);
  Title_Print("Test vert_value");
  for (int i = 0; i < n_verts; i++) {
    // std::cout << "  v"+std::to_string(i)+" = " << *(float *)&vert_value[i] << " "<< *((float *)(&vert_value[i])+1) << " "<< *((float *)(&vert_value[i])+2) << " " << std::endl;
    std::cout <<  "  v"+std::to_string(i)+" = " << (float *)&vert_value[i] << std::endl;
  }
